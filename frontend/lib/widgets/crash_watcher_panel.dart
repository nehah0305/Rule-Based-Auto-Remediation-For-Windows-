        ...
        Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            if (!watching) ...[
              ...
              const _InstructionStep(num: '1', text: 'Toggle "Watch for Crashes" to arm the watcher'),
              const SizedBox(height: 8),
              const _InstructionStep(num: '2', text: 'Open Notepad (Win + R → notepad)'),
              const SizedBox(height: 8),
              const _InstructionStep(num: '3', text: 'Kill Notepad via Task Manager → End Task'),
              const SizedBox(height: 8),
              const _InstructionStep(num: '4', text: 'Watch the crash popup appear instantly ⚡'),
              ...
              const Row(children: [
                ...
                SizedBox(width: 12),
                Text('Polling every 3 seconds…', style: TextStyle(color: AppTheme.textMuted, fontSize: 12)),
              ]),
              const SizedBox(height: 12),
              const _StatusRow(icon: Icons.search, label: 'Monitoring', value: 'notepad.exe', color: Color(0xFFef4444)),
              const SizedBox(height: 6),
              const _StatusRow(icon: Icons.event_note, label: 'Event ID', value: '1000 (Application Error)', color: AppTheme.accent),
              const SizedBox(height: 6),
              const _StatusRow(icon: Icons.timer_outlined, label: 'Look-back window', value: '60 seconds', color: AppTheme.accentGreen),
            ],
          ]),
        ),
      ]),
...
class _PulsingDotState extends State<_PulsingDot> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    if (!widget.active) {
      return Container(
        width: 10, height: 10,
        decoration: const BoxDecoration(color: AppTheme.textDimmed, shape: BoxShape.circle),
      );
    }
