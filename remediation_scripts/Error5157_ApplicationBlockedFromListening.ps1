# Remediation Script for Event ID 5157: Application blocked from listening
# Conservative remediation: verify the firewall service and optionally create a scoped allow rule.
...
$LOG_FILE = 'C:\Temp\Event5157_Remediation.log'
...
Write-Log 'Event 5157 received: Application blocked from listening.'
...
if ($SIMULATION_MODE) {
    Write-Log ("[SIMULATION] Would verify firewall service and evaluate an allow rule for: {0}" -f $appName)
    ...
    try {
        ...
        if ($ALLOW_FIREWALL_RULES -and $appName) {
            $ruleName = "AutoAllow-$appName-Listen"
            $existing = Get-NetFirewallRule -DisplayName $ruleName -ErrorAction SilentlyContinue
            if (-not $existing) {
                New-NetFirewallRule -DisplayName $ruleName -Direction Inbound -Action Allow -Program $appName -Profile Any -ErrorAction SilentlyContinue | Out-Null
        ...
        Write-Log ("Firewall listen-block remediation failed: {0}" -f $_.Exception.Message)
        $action = 'partial'
    }
}

$status = if ($action -in @('simulated-evaluate','allow-rule-evaluated','firewall-checked')) { 'SUCCESS' } else { 'PARTIAL' }
$resolutionMessage = "Auto-remediation $status for Event 5157. App=$appName Action=$action Time=$timestamp"
