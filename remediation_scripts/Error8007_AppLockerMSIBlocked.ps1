# Remediation Script for Event ID 8007: AppLocker MSI installer blocked
...
$LOG_FILE = 'C:\Temp\Event8007_Remediation.log'
...
function Get-BlockedMsi {
    param([string]$Message)

    if ($Message -match '([A-Za-z]:\\[^,\r\n]+\.msi)') { return $matches[1].Trim() }
    if ($Message -match 'MSI\s+([^,\r\n]+\.msi)') { return $matches[1].Trim() }
    return $null
}
...
Write-Log 'Event 8007 received: AppLocker MSI installer blocked.'
Write-Log ("Message: {0}" -f $message)

$blockedMsi = Get-BlockedMsi -Message $message
$action = 'none'

if ($SIMULATION_MODE) {
    Write-Log ("[SIMULATION] Would refresh AppLocker policy and review blocked MSI: {0}" -f $blockedMsi)
    ...
    try {
        ...
        Write-Log ("AppLocker MSI remediation failed: {0}" -f $_.Exception.Message)
        $action = 'partial'
    }
}

$status = if ($action -in @('simulated-review','refreshed-policy')) { 'SUCCESS' } else { 'PARTIAL' }
$resolutionMessage = "Auto-remediation $status for Event 8007. BlockedMSI=$blockedMsi Action=$action Time=$timestamp"
