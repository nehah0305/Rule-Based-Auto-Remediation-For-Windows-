# Remediation Script for Event ID 8006: AppLocker script execution blocked
...
$LOG_FILE = 'C:\Temp\Event8006_Remediation.log'
...
function Get-BlockedScript {
    param([string]$Message)

    if ($Message -match '([A-Za-z]:\\[^,\r\n]+\.(ps1|vbs|js|cmd|bat))') { return $matches[1].Trim() }
    if ($Message -match 'script\s+([^,\r\n]+\.(ps1|vbs|js|cmd|bat))') { return $matches[1].Trim() }
    return $null
}
...
Write-Log 'Event 8006 received: AppLocker script execution blocked.'
Write-Log ("Message: {0}" -f $message)

$blockedScript = Get-BlockedScript -Message $message
$action = 'none'

if ($SIMULATION_MODE) {
    Write-Log ("[SIMULATION] Would refresh AppLocker policy and review blocked script: {0}" -f $blockedScript)
    ...
    try {
        ...
        Write-Log ("AppLocker script remediation failed: {0}" -f $_.Exception.Message)
        $action = 'partial'
    }
}

$status = if ($action -in @('simulated-review','refreshed-policy')) { 'SUCCESS' } else { 'PARTIAL' }
$resolutionMessage = "Auto-remediation $status for Event 8006. BlockedScript=$blockedScript Action=$action Time=$timestamp"
